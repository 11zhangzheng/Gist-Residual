...
Gist 用于全视频粗召回，不要求覆盖颜色、计数、精细空间关系等细节。必须保留 `embedding + ASR only` 变体，用于验证短文本 Gist 的收益是否值得其构建成本。
...
Residual 使用比 Gist 更高的帧密度和分辨率，但不直接回答当前问题。生成结果持久化到事件级缓存。
...
首版不允许 Router 输出任意帧数。Visual 结果缓存精确问题的验证答案，同时缓存可复用的帧选择与事件观察。

### 4.4 Context

`EXPAND_CONTEXT(event)` 获取目标事件前后的相邻 Gist，以及已经存在的相邻 Residual。它不隐式触发新的高成本 Residual 或 Visual 操作，避免隐藏成本。
...
若所有路径均答错，则先按答案置信度排序，再选择最低成本路径。Multiple-choice 任务使用确定性精确判分；Answerer 使用 temperature 0。
...
训练集包含 Oracle 路径上的状态–动作对，以及近优但错误路径的状态作为负例。

### 10.2 DAgger 式纠偏

让 BC Router 在训练环境中运行，对其偏离 Oracle 后访问的状态重新查询缓存搜索器并标注，以降低序列误差累积。

### 10.3 Offline RL

Offline RL 是可选扩展。只有 BC+DAgger 已达到核心门槛，且主要剩余误差明确来自长期成本分配时才运行；不得为了 RL 重新在线生成大规模 VLM 观察。
...
2. Prompt/VLM Agent：同 action space 的 Prompt Controller，以及可公平复现的 VideoARM/VCA 风格策略；
...
| 周期 | 交付物 | 决策门槛 |
|---|---|---|
| 第1–2周 | 数据类型、切分、缓存、成本计量、toy pipeline | 单视频可运行，成本可复算 |
...
- Oracle 原子观察超过 A800 核心预算的 45% 时，先降低 K、训练题数或 Visual 生成比例。
