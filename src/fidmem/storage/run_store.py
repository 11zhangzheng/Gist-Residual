...
class RunItem:
    ...
    output_uri:str|None; error_type:str|None; error_message:str|None
class RunStore:
    ...
    def claim(self,run_id:str,item_key:str,worker_id:str)->bool:
        with self._connect() as c:
            ...
            try:
                ...
                result=c.execute("UPDATE run_items SET status='running',attempt=attempt+1,worker_id=?,started_at=?,finished_at=NULL,error_type=NULL,error_message=NULL WHERE run_id=? AND item_key=? AND status IN ('pending','failed') RETURNING item_key",[worker_id,_utcnow(),run_id,item_key]).fetchone();c.execute("COMMIT")
    ...
    def item(self,run_id:str,item_key:str)->RunItem|None:
        with self._connect() as c:row=c.execute("SELECT run_id,item_key,status,attempt,output_uri,error_type,error_message FROM run_items WHERE run_id=? AND item_key=?",[run_id,item_key]).fetchone()
        return None if row is None else RunItem(*row)
    def items(self,run_id:str)->tuple[RunItem,...]:
        with self._connect() as c:rows=c.execute("SELECT run_id,item_key,status,attempt,output_uri,error_type,error_message FROM run_items WHERE run_id=? ORDER BY item_key",[run_id]).fetchall()
